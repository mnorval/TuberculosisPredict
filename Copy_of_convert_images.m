...
%categories = {'No_TB128x','Has_TB128x'};
rootFolder = 'E:\Documents\Dropbox\Personal\Study\MTech\Simulations\Matlab\Datasets\Adjusted_Cropped_Has_TB1024_RGB';
...
[xray_image,info] = readimage(imagedata,1);


%subplot(sub_plot_y,sub_plot_x,sub_plot_count), imshow(xray_image), title(['original image: ',num2str(image_number)]);sub_plot_count=sub_plot_count+1;

xray_image_grey =  rgb2gray(xray_image);
%xray_image_grey = imadjust(xray_image_grey);
%xray_image_grey(xray_image_grey<100) = 127;
%xray_image_grey = imclose(xray_image_grey,strel('disk',1))
subplot(sub_plot_y,sub_plot_x,sub_plot_count), imshow(xray_image_grey), title('Grey Adjusted');sub_plot_count=sub_plot_count+1;

xray_image_binary = (xray_image_grey > 140) & (xray_image_grey < 300);
xray_image_binary = imclose(xray_image_binary,strel('disk',2))
%subplot(sub_plot_y,sub_plot_x,sub_plot_count), imshow(xray_image_binary), title('Binary Image');sub_plot_count=sub_plot_count+1;
...
%subplot(sub_plot_y,sub_plot_x,count), imshow(roi_image), title('Area');count=count+1;

roi_image = ~roi_image;
BW2 = bwareafilt(roi_image,[100 5500]);
%subplot(sub_plot_y,sub_plot_x,count), imshow(BW2), title('Extracted');count=count+1;
...
%try
    for tempcount = 1:20
        ...
        if (SC(tempcount).Centroid(1)) > 1 && (SC(tempcount).Centroid(1) < 200) 
...
subplot(sub_plot_y,sub_plot_x,count), imshow(right_lung_image), title('Right Lung');count=count+1;
